...
def display_database_system(database_name, table_name, entry_columns, labels, home_page_function):
    ...
    def add():
        ...
        if any(not data.strip() for data in entry_values):
            ...
            try:
                conn = connection(database_name)
                cursor = conn.cursor()
                cursor.execute(f"INSERT INTO {table_name} VALUES ({', '.join(['%s']*len(entry_values))})", tuple(entry_values))
                conn.commit()
                conn.close()
            except pymysql.IntegrityError:
                messagebox.showinfo("Error", f"{table_name.capitalize()} already exists.")
                return
    ...
    def reset():
        decision = messagebox.askquestion("Warning!!", f"Delete all data from {table_name.capitalize()}?")
        if decision == "yes":
            try:
                conn = connection(database_name)
                cursor = conn.cursor()
    ...
    def delete():
        decision = messagebox.askquestion("Warning!!", f"Delete the selected data from {table_name.capitalize()}?")
        if decision == "yes":
            try:
                conn = connection(database_name)
                cursor = conn.cursor()
                ...
                cursor.execute(f"DELETE FROM {table_name} WHERE {table_name.lower()}ID = %s", (delete_data,))
                conn.commit()
                conn.close()
            except:
                messagebox.showinfo("Error", "Sorry, an error occurred.")
    ...
    def update():
        decision = messagebox.askquestion("Warning!!", f"Update the selected data from {table_name.capitalize()}?")
        if decision == "yes":
            try:
                selected_item = my_tree.selection()[0]
    ...
    def refresh_table():
        ...
        my_tree.grid(row=4, column=0, columnspan=5, rowspan=5, padx=10, pady=10)
    ...
    input_frame.grid(row=3, column=0, padx=300, pady=30, columnspan=5, sticky="w")
    ...
    for i, label_text in enumerate(labels):
        label = tk.Label(input_frame, text=label_text, font=('Helvetica', 18, 'bold'), bg="#A9DFBF", fg="black")
    ...
    tree_frame.grid(row=4, column=0, padx=150, pady=0, columnspan=5, sticky="nsew")
