...
from Components import display_database_system, display_query_result
...
def home_page(root):
    ...
    label_home = Label(home_frame, text="Welcome to Fifa Women's World Cup 2023 Database", font=(
        'Georgia', 45, 'bold'))
    label_home.grid(row=2, column=1, pady=60, padx=65)

    # Buttons to navigate to different pages
    player_button = Button(home_frame, text="Player Profile", command=player_page,
                           width=30, padx=15, pady=10, font=('Helvetica', 16, 'bold'), border=10)
    player_button.grid(row=10, column=1)

    team_button = Button(home_frame, text="Team Information", command=team_page,
                         width=30, padx=15, pady=10, font=('Helvetica', 16, 'bold'), border=10)
    team_button.grid(row=40, column=1)

    venue_button = Button(home_frame, text="Venue Details", command=venue_page,
                          padx=15, width=30, pady=10, font=('Helvetica', 16, 'bold'), border=10)
    venue_button.grid(row=70, column=1)

    matches_button = Button(home_frame, text="Match Details", command=matches_page,
                            padx=15, width=30, pady=10, font=('Helvetica', 16, 'bold'), border=10)
    matches_button.grid(row=100, column=1)

    goal_button = Button(home_frame, text="Goal Details", command=goal_page,
                         padx=15, width=30, pady=10, font=('Helvetica', 16, 'bold'), border=10)
    goal_button.grid(row=130, column=1)

    fixture_button = Button(home_frame, text="Fixture Details", command=fixture_page,
                            padx=15, width=30, pady=10, font=('Helvetica', 16, 'bold'), border=10)
    fixture_button.grid(row=170, column=1)

    standing_button = Button(home_frame, text="Standing Details", command=standing_page,
                             padx=15, width=30, pady=10, font=('Helvetica', 16, 'bold'), border=10)
    standing_button.grid(row=180, column=1)

    advanced_button = Button(home_frame, text="Advanced Functions", command=calculate_total_goals,
                             padx=15, width=30, pady=10, font=('Helvetica', 16, 'bold'), border=10)
    advanced_button.grid(row=190, column=1)

# Calculate the total goals scored by each team


def calculate_total_goals():
    # Use SQL query to calculate total goals by each team
    query1 = """
        SELECT
            t.TeamName,
    ...
    database_name = "Player_Demo"
    display_query_result([query1, query2, query3], [["TeamName", "Total Goals"], [
                         "TeamID", "Points", "TeamRank"], ["TeamID", "MatchDate", "RunningTotalGoals"]], database_name, home_page)
...
def venue_page():
    ...
    display_database_system("Player_Demo", "Venue",
                            venue_columns, venue_labels, home_page)
...
def player_page():
    ...
    display_database_system("Player_Demo", "Player",
                            player_columns, player_labels, home_page)
...
def team_page():
    ...
    team_columns = ["ID", "Team Name", "Country", "Coach",
                    "Formation", "FIFA Ranking", "Matches Played"]

    team_labels = ["ID", "Team Name", "Country", "Coach",
                   "Formation", "FIFA Ranking", "Matches Played"]

    display_database_system("Player_Demo", "Team",
                            team_columns, team_labels, home_page)
...
def matches_page():
    ...
    match_columns = ["MatchesID", "MatchesDate", "Location",
                     "TeamID1", "TeamID2", "Result", "VenueID"]

    match_labels = ["MatchesID", "MatchesDate", "Location",
                    "TeamID1", "TeamID2", "Result", "VenueID"]

    display_database_system("Player_Demo", "Matches",
                            match_columns, match_labels, home_page)
...
def goal_page():
    ...
    display_database_system("Player_Demo", "Goal",
                            goal_columns, goal_labels, home_page)
...
def fixture_page():
    ...
    display_database_system("Player_Demo", "Fixture",
                            fixture_columns, fixture_labels, home_page)
...
def standing_page():
    ...
    standing_columns = ["StandingID", "TeamID",
                        "Wins", "Draws", "Losses", "Points"]

    standing_labels = ["StandingID", "TeamID",
                       "Wins", "Draws", "Losses", "Points"]

    display_database_system("Player_Demo", "Standing",
                            standing_columns, standing_labels, home_page)
